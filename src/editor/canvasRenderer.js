...
export class CanvasRenderer {
  ...
  render() {
    ...
    if (this.state.showRegionOverlay !== false) {
      for (const reg of this.state.regions) {
        const isActive = reg.id === this.state.activeRegionId;
        ctx.strokeStyle = isActive ? 'rgba(56, 189, 248, 0.75)' : 'rgba(56, 189, 248, 0.35)';
        ctx.lineWidth = isActive ? 0.4 : 0.2;
        ctx.fillStyle = isActive ? 'rgba(56, 189, 248, 0.04)' : 'rgba(56, 189, 248, 0.015)';

        if ((reg.type === 'polygon' || reg.points) && reg.points && reg.points.length >= 3) {
          ctx.beginPath();
        ...
        // Sleek semi-transparent Region Label Badge
        const labelW = Math.max(20, (reg.name || 'Local Area').length * 2.0);
        const labelX = (reg.type === 'polygon' || reg.points) && reg.points ? reg.points[0].x : reg.x;
        const labelY = (reg.type === 'polygon' || reg.points) && reg.points ? reg.points[0].y : reg.y;

        ctx.fillStyle = isActive ? 'rgba(2, 132, 199, 0.75)' : 'rgba(15, 23, 42, 0.65)';
        ctx.fillRect(labelX, Math.max(0, labelY - 4.0), labelW, 4.0);
        ctx.fillStyle = '#ffffff';
        ctx.font = '2.0px sans-serif';
        ctx.fillText(reg.name || 'Local Area', labelX + 1.0, Math.max(2.8, labelY - 1.0));
      }
    }
